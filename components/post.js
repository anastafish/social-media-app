import { Card, CardHeader, CardBody, CardFooter, Flex, Avatar, Box, Heading, Text, Button} from '@chakra-ui/react'
...
export default function Post({name, text, id, post, likes}) 
{
  ...
  useEffect(() => {
    get(child(starCountRef, `/users/${auth.currentUser.uid}`)).then((snapshot) => {
      if (snapshot.exists()) {
        setLikedPosts(snapshot.val().liked.split(','))
      } else {
  ...
  function like(e){
    if(!likedPosts.includes(e.target.id)){
      setLikedPosts(prevState => [...prevState, e.target.id])
      get(child(starCountRef, `/users/${auth.currentUser.uid}`)).then((snapshot) => {
        if (snapshot.exists()) {
          set(ref(db, `/users/${auth.currentUser.uid}`), {
          ...snapshot.val(),
          liked:snapshot.val().liked + ',' + e.target.id
        });
    ...
    get(child(starCountRef, `/posts/post${e.target.id}`)).then((snapshot) => {
      if (snapshot.exists()) {
    ...
    setLikedPosts(prevState => {
      const index = prevState.indexOf(e.target.id);
      if (index > -1) {
  ...
  >
        <div className='flex w-full justify-between'>
          <div className='flex flex-col items-center justify-center'>
            <Image src={likedPosts.includes(id) ? filled_like : like_icon} style={{width:'30px'}} id={id} onClick={like} alt='like-button'/>
            <h1>{likes}</h1>
          </div>
          <Image src={comment} style={{width:'30px'}} alt='comment-button'/>
          <Image src={share} style={{width:'30px'}} alt='share-button'/>
          {user.displayName === name && <Image variant='ghost' onClick={delPost} id={id} src={bin} style={{width:'30px'}} alt='delete-button'/>}
        </div>
  </CardFooter>
</Card>
    )
